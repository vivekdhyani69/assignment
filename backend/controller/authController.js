...
const login = async (req, res) => {
  ...
  try {
      ...
      //If credential is correct then makes jwt.sign token
...
const resetTokenExpiry = Math.floor(Date.now() / 1000) + 600; // 600 seconds for 10 minutes
 ...
 const resetLink = `https://etlhive-assignment.vercel.app/reset-password/${resetToken}`;
...
const resetPassword = async(req,res)=>{
  const { token } = req.params;///In a params takes token 
