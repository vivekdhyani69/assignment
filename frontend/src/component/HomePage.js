...
const HomePage = () => {
  ...
  const fetchLeads = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/leads/getleads/${userData._id}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSearchChange = (e) => {
    ...
    timer = setTimeout(() => {
      if (query) {
        const filteredLeads = allLeads && allLeads.filter(lead =>
          lead.title?.toLowerCase().includes(query.toLowerCase()) ||
          lead.name?.toLowerCase().includes(query.toLowerCase()) ||
          lead.price?.includes(query) 
...
// Function to handle sorting order change
 ...
 const handleSortOrderChange = async (order) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/leads/sort/${userData._id}?sortBy=${order}`);

      setLeads(response.data.leads); // Update Book with sorted data
    } catch (err) {
  ...
  return (
    <div className="lead-details-container">
      ...
      <Link to="/create-lead">
        <button className="back-button">Create Book</button>
      </Link>
    </div>
  );
};
